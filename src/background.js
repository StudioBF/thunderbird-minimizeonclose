(async function () {
  ...
  const windowClosingCallback = async function (windowId) {
    ...
    await messenger.ex_windows_revamped.unlockWindow(windowId);
    await messenger.windows.remove(windowId);
  };
  await messenger.ex_windows_revamped.onClosing.addListener(windowClosingCallback);
  ...
  const windowRegistrationListener = async function (windowInfo) {
    if (windowInfo.id && windowInfo.type === "normal") {
      ...
      await messenger.ex_windows_revamped.lockWindow(windowInfo.id);
    }
  }
